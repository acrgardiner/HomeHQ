...
public partial class EditAttachmentType
{
    [CascadingParameter] public required IMudDialogInstance Dialog { get; set; }

    [Parameter] public required AttachmentType Entity { get; set; }
    [Parameter] public required string Title { get; set; }

    private MudForm _form;
    private AttachmentType _model = new();
    ...
    private async Task Submit()
    {
        ...
        if (_form.IsValid)
        {
            try
            {
                await AttachmentService.UpdateAsync(_model);
